...
import { AssignGithubIssueSchema, CreateGithubIssueCommentSchema, CreateGithubIssueSchema, CreateGithubLabelSchema, GetAllGithubIssuesSchema, UpdateGithubIssueSchema } from "@repo/types";
...
GithubRouter.get("/user", authMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (!user || !user.githubToken) {
            return res.status(404).json({ error: "GitHub access token not found for user" });
        }
...
GithubRouter.post("/create/issue",  authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const repoOwner = owner || user.githubUsername;
...
GithubRouter.post("/issue/comment", authMiddleware, async (req, res) => {
    ...
    const { owner, repo, issueNumber, comment } = CreateGithubIssueCommentSchema.parse(req.body);
    ...
    try {
        ...
        const { data } = await axios.post(
            `https://api.github.com/repos/${repoOwner}/${repo}/issues/${issueNumber}/comments`,
            { body: comment },
            {
                headers: {
                    Authorization: `Bearer ${user?.githubToken}`,
...
GithubRouter.patch("/issue/update", authMiddleware, async (req, res) => {
    ...
    const { owner, repo, issueNumber, state, title, body } = UpdateGithubIssueSchema.parse(req.body);
...
GithubRouter.post("/issue/assign", authMiddleware, async (req, res) => {
    ...
    const { owner, repo, issueNumber, assignees } = AssignGithubIssueSchema.parse(req.body);

    if (!owner || !repo || !issueNumber || !assignees) {
        return res.status(400).json({ error: "Missing params" });
    }
    ...
    try {
        ...
        const { data } = await axios.post(
            `https://api.github.com/repos/${repoOwner}/${repo}/issues/${issueNumber}/assignees`,
            { assignees },
            {
                headers: {
                    Authorization: `Bearer ${user?.githubToken}`,
...
GithubRouter.post("/issue/labels", authMiddleware, async (req, res) => {
    ...
    const { owner, repo, issueNumber, labels } = CreateGithubLabelSchema.parse(req.body);
    ...
    try {
        ...
        const { data } = await axios.post(
            `https://api.github.com/repos/${repoOwner}/${repo}/issues/${issueNumber}/labels`,
            { labels },
            {
                headers: {
                    Authorization: `Bearer ${user?.githubToken}`,
